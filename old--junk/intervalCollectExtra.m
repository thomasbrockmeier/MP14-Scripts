...
temp = zeros(10, 1);
intervals = zeros(size(onsets) * smoothing - 1);
for iii = 1:length(onsets)
    for j = 1:smoothing
        countJ = countJ + 1;
        try
        plop = pitchesKeys(str2double(num2str(round(onsets(countJ) * 100))));
        catch err
        end
    end
    
    % Try to discern 0 intervals from 1 intervals
%     if mean(temp(~isnan(temp))) == 0
%         tempje = temp(~isnan(temp));
%         tempje = tempje(tempje ~= 0);
%         if isempty(tempje)
%             temp(1) = 0;
%         else
%             temp(1) = mean(tempje);
...
%     end
    ...
    % boundary)
    if iii > 1
        if abs(pitchOn(iii) - pitchOn(iii - 1)) == 12
            intervals(iii - 1) = 12;
        else
